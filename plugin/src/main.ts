...
export default class ObsidianKrakePlugin extends Plugin {
    private dashboard_view: OkDashboardView;
    ...
    async onload() {
        ...
        this.registerView(
            ...
            (leaf: WorkspaceLeaf) =>
                (this.dashboard_view = new OkDashboardView(leaf, this.app))
        );

        this.addRibbonIcon('krake', 'OK Dashboard', async (evt: MouseEvent) => {
            const workspace = this.app.workspace;
            workspace.detachLeavesOfType(OkDashboardView.getViewTypeName());
            const leaf = workspace.getLeaf(!Platform.isMobile);
            await leaf.setViewState({
                type: OkDashboardView.getViewTypeName(),
            });
