<script lang="ts">
	...
	import { days_ago_text, get_collection } from '$lib/helper';
	...
	export let suggest_project: (exclude_paths: string[]) => Promise<Omit<Parent, 'parents'>>;
	export let suggest_topic: (exclude_paths: string[]) => Promise<Omit<Parent, 'parents'>>;
	export let move_file: (from_path: string, to_path: string) => Promise<void>;
	export let delete_file: (file_path: string) => Promise<void>;

	$: entry_type = (path.startsWith(paths.task) ? 0 : path.startsWith(paths.project) ? 1 : 2) as
		| 0
		| 1
		| 2;
	$: color = entry_type === 0 ? 'teal' : entry_type === 1 ? 'violet' : 'pink';
	$: entry = get_collection($db, entry_type).find((t) => t.file_path === path) as
		| (Entry & { do_date?: Date; due_date?: Date })
		| undefined;
	...
	async function add_project_parent() {
		...
		const parent_info = await suggest_project(entry.parents.map((p) => p.file_path));
	...
	async function add_topic_parent() {
		...
		const parent_info = await suggest_topic(entry.parents.map((p) => p.file_path));
		await db.add_parent(entry, parent_info);
	...
	async function change_type(to: 0 | 1 | 2) {
		...
		const new_path = entry.file_path.replace(map_path(entry.type), map_path(to));
	...
	{#if entry}
		<div class="flex items-center justify-between">
			<div class="flex items-center gap-2">
				...
				<Path parents={entry.parents} {open} can_remove on:remove={remove_parent} />
			</div>

			<div class="flex gap-1">
				<div class="flex items-center gap-1 bg-neutral-100 rounded-lg px-1 py-1.5 text-sm">
				...
				{#if entry_type !== 2}
					...
					<DateIndicator due bind:date={entry.due_date} on:changed={save} />
				{/if}
		...
		{#if actions_visible}
			<div class="flex justify-end gap-2 items-enter">
				...
				<IconButton id="entry_picker" color="sky" on:click={() => (show_entry_picker = true)}>
					<Pen />
				</IconButton>
