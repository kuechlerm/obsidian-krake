...
export const drop = (entry: Entry, dragging: Entry | null) => async (e: Event) => {
...
export const drag_over = (entry: Entry, dragging: Entry | null) => (e: Event) => {
	if (!entry.file_path.startsWith(paths.task) && entry.file_path !== dragging?.file_path)
