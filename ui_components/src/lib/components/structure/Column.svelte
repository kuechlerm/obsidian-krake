<script lang="ts">
	...
	import { byStringProperty, get_collection } from '$lib/helper';
	...
	$: children = (parent_entry?.children ?? [])
		.map((c) => get_collection($db, c.type).find((e) => e.file_path === c.file_path))
...
<div class="flex flex-col gap-2 border border-solid p-2 rounded-lg bg-neutral-100 min-w-[100px]">
		...
		>
			<div class="flex items-center gap-1">
				<div class="w-5 flex items-center">
					{#if entry.type === 0}
						<Checkbox checked={!!entry.done} on:change={toggle_done(entry)} />
