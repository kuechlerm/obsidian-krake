<script lang="ts">
	...
	import type { Task as TaskType, TaskBlockConfig } from '$lib/types';
	...
	function filter_tasks(tasks: TaskType[], config: TaskBlockConfig) {
		...
		if (config.do_date) {
			...
			filtered = filtered.filter((task) => !!task.do_date && date_filter(task.do_date));
		}

		if (config.due_date) {
			const date_filter = create_date_filter(config.due_date);
			filtered = filtered.filter((task) => !!task.due_date && date_filter(task.due_date));
		}
